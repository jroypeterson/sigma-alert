...
| ETF names | `sources/etf_names.json` | `{TICKER: friendly name}` for index + sector + healthcare + tech + commodity + macro tickers; pharma rows labeled by composition (large-cap vs diversified). Coverage Manager doesn't maintain this metadata, so display names live here and get merged into the metadata copy at startup |
