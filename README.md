...
7. **Every digest**: an **Index, Sector & Macro Returns** section at the bottom. A `_Macro_` sub-header leads (10Y Treasury yield `^TNX` as `level% | ±bp | YTD ±bp from year-start`, US Dollar Index `DX-Y.NYB` and WTI crude `CL=F` as `level | ±% | YTD ±%` — the cross-asset backdrop, from `sources/macro.txt`). Then broad-market indices (SPYM = S&P 500, DIA = Dow Jones, QQQ = Nasdaq 100, plus the `^W5000` Wilshire 5000 whole-market index) under `_Indices_`, then SPDR sector ETFs (XLV, XLK, XLE, …) under `_Sectors_`, then healthcare sub-sector ETFs (XBI, IBB, IHI, XHS, PPH, IHE) plus the `^DRG` NYSE Arca Pharmaceutical index under `_Healthcare_`, then tech-theme ETFs (`MAGS` Mag-7, `SMH`/`SOXX` semis, `IGV` software, `DTCR` data-center REITs, `AIQ` AI/tech) under `_Tech Themes_`. The index/sector/healthcare/tech groups sort by z-score descending; macro renders in a fixed rates→FX→commodity order. Each **ETF** row appends prior calendar year and YTD returns — e.g. `| 2025: +24.50% | YTD: +6.10%`; macro rows append a YTD figure too (WTI/dollar index as a YTD %, the 10Y yield as a YTD bp move from its year-start level). The screener unions `sources/index_etfs.txt`, `sources/sector_etfs.txt`, `sources/healthcare_etfs.txt`, `sources/tech_etfs.txt`, and `sources/macro.txt` into the download list at startup so the section keeps rendering even if a watchlist sync drops them from `watchlist.txt`. Today's column reuses already-downloaded data; the period columns come from a dedicated ~800-day fetch covering the ETFs, alert tickers, and macro tickers (so the main 400-day batch download stays small)
