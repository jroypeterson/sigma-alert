...
7. **Every digest**: an **Index & Sector Returns** section at the bottom. Broad-market index ETFs (SPYM = S&P 500, DIA = Dow Jones, QQQ = Nasdaq 100) render first under an `_Indices_` sub-header, then SPDR sector ETFs (XLV, XLK, XLE, …) under `_Sectors_`, then healthcare sub-sector ETFs (XBI, IBB, IHI, XHS, PPH) under `_Healthcare_`. All three groups sort by z-score descending. Each ETF row appends prior calendar year and YTD returns — e.g. `| 2025: +24.50% | YTD: +6.10%`. The screener unions `sources/index_etfs.txt`, `sources/sector_etfs.txt`, and `sources/healthcare_etfs.txt` into the download list at startup so the section keeps rendering even if a watchlist sync drops the ETFs from `watchlist.txt`. Today's column reuses already-downloaded data; the period columns come from a dedicated ~800-day ETF-only fetch (so the main 400-day batch download stays small)
...
| Healthcare ETFs | `sources/healthcare_etfs.txt` | Healthcare sub-sector ETFs (XBI, IBB, IHI, XHS, PPH). Render below the SPDR sectors under a `_Healthcare_` sub-header in the same section |
| ETF names | `sources/etf_names.json` | `{TICKER: friendly name}` for index + sector + healthcare ETFs. Coverage Manager doesn't maintain ETF metadata, so display names live here and get merged into the metadata copy at startup |
