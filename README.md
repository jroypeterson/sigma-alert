...
7. **Every digest**: an **Index & Sector Returns** section at the bottom. Broad-market index ETFs (SPYM = S&P 500, DIA = Dow Jones, QQQ = Nasdaq 100) render first under an `_Indices_` sub-header, then SPDR sector ETFs (XLV, XLK, XLE, …) under `_Sectors_`. Both groups sort by z-score descending. Each ETF row appends prior calendar year and YTD returns — e.g. `| 2025: +24.50% | YTD: +6.10%`. The screener unions `sources/index_etfs.txt` and `sources/sector_etfs.txt` into the download list at startup so the section keeps rendering even if a watchlist sync drops the ETFs from `watchlist.txt`. Today's column reuses already-downloaded data; the period columns come from a dedicated ~800-day ETF-only fetch (so the main 400-day batch download stays small)
