...
- **Reads:** yfinance OHLCV · Coverage Manager `ticker_metadata.json` + position lists + S&P 500 list (`sources/sp500*.{txt,json}`)
...
6. **Close report only**: flags any ticker that hit a new 52-week high or low during the session, **grouped by the same subcategory taxonomy as the alert tiers** (Portfolio → … → MedTech → HC Services → S&P 500) with an `Uncategorized` catch-all so nothing is hidden
7. **Every digest**: an **Index, Sector & Macro Returns** section at the bottom. A `_US Indices_` sub-header **leads** (broad-market indices SPYM = S&P 500, DIA = Dow Jones, QQQ = Nasdaq 100, the `^W5000` Wilshire 5000 whole-market index, and the `^RUT` Russell 2000 small-cap index — US index price returns are the headline, sorted by z-score; renamed from `_Indices_` and moved to the top on 2026-06-29). Then a `_Macro_` sub-header (10Y Treasury yield `^TNX` as `level% | ±bp | YTD ±bp from year-start`, US Dollar Index `DX-Y.NYB` and WTI crude `CL=F` as `level | ±% | YTD ±%` — the cross-asset backdrop, from `sources/macro.txt`), closing with the **30-year fixed mortgage rate** (Freddie Mac PMMS via FRED `MORTGAGE30US`, as `level% | ±bp w/w | YTD ±bp from year-start | as of DATE`, colored bond-style). **That series is weekly, not daily** — hence the explicit `w/w` on the change and the inline observation date, so a value re-shown across the week can't read as today's move. Then a `_Credit_` sub-header — HY & IG **effective yields + OAS spreads** from FRED's public no-key CSV (`yield X.XX% ±bp | OAS Zbp ±bp | YTD: yield ±bp, OAS ±bp`, HY→IG order, colored by the spread move: widening = 🟥, tightening = 🟩; the two YTD changes are labeled separately so the absolute-bp move is unambiguous). Then global/international equity index ETFs (`ACWI` world, `EFA` developed ex-US, `EEM` EM, `VGK` Europe, `EWJ`/`EWY`/`FXI`/`INDA` Japan/Korea/China/India) under `_Global Equity_`, then SPDR sector ETFs (XLV, XLK, XLE, …) under `_Sectors_`, then healthcare sub-sector ETFs (XBI, IBB, IHI, XHS, PPH, IHE) plus the `^DRG` NYSE Arca Pharmaceutical index under `_Healthcare_`, then tech-theme ETFs (`MAGS` Mag-7, `SMH`/`SOXX` semis, `IGV` software, `DTCR` data-center REITs, `AIQ` AI/tech) under `_Tech Themes_`, then commodity ETFs (`GLD` Gold) under `_Commodities_`. The index/sector/healthcare/tech/commodity groups sort by z-score descending; macro and credit render in a fixed order. Each **ETF** row appends prior calendar year and YTD returns — e.g. `| 2025: +24.50% | YTD: +6.10%`; macro rows append a YTD figure too (WTI/dollar index as a YTD %, the 10Y yield as a YTD bp move from its year-start level); credit rows show both YTD moves (yield + OAS) in bp, each labeled. The screener unions `sources/index_etfs.txt`, `sources/sector_etfs.txt`, `sources/healthcare_etfs.txt`, `sources/tech_etfs.txt`, `sources/commodity_etfs.txt`, and `sources/macro.txt` into the download list at startup so the section keeps rendering even if a watchlist sync drops them from `watchlist.txt`. Today's column reuses already-downloaded data; the period columns come from a dedicated ~800-day fetch covering the ETFs, alert tickers, and macro tickers (so the main 400-day batch download stays small); credit comes from a separate bounded FRED fetch (warn-and-proceed — the `_Credit_` block is simply omitted if FRED is unreachable)
...
Short names for S&P 500 tickers that Coverage Manager doesn't maintain come from `sources/sp500_names.json`, a Wikipedia-sourced fallback written by Coverage Manager's weekly sigma-export (since 2026-09-22; `scripts/refresh_sp500.py` is a manual fallback).
...
## Return map (periodic table of returns)

`scripts/return_map.py` renders an interactive **return map** — a BlackRock-style periodic-table-of-returns — for the index/asset groups the returns block already tracks (US Indices, Global Equity, Sectors, Healthcare, Tech Themes, Commodities, Macro). It is **purely derived from the returns the screener has already computed** — no extra market-data fetch.

Each screener run **that clears the return-map coverage gate** (see below) writes `cache/returns_snapshot.json` (the returns block packed to JSON) and rebuilds `readable/return_map.html`, three ranked columns (Prior Year / YTD / Today) with tiles colored by asset class, hover detail, and a click-to-isolate legend. The HTML is self-contained (no CDN/libraries — opens straight off disk, works offline). Snapshot persistence is warm-fail: if it errors, the alert pipeline is unaffected.

**Coverage gates (2026-09-07).** Yahoo throttles the CI egress IPs, and from 2026-09-03 the close cycle screened 0/769, then 0/757, then 34/757 — and still posted, carrying 2 alerts drawn from 4.8% of the market. Two floors now stop that. A run screening under **80%** of the watchlist reports `error` and publishes **nothing**: no Slack digest, no return-map rewrite. Between 80% and 95% it publishes with a `DEGRADED RUN` banner naming the real coverage and reports `partial`. The return map is gated **separately**, on its own ~43-symbol universe — the watchlist floor cannot see an ETF-only outage, and rewriting `return_map.html` with an empty map would reset its mtime and make a broken lane look freshly updated to the artifact-freshness monitor. The health heartbeat always posts, and now reports whether the digest was actually **delivered**, not merely attempted.

The snapshot is committed by `sigma-close.yml`; the rendered HTML is a derived runtime artifact (`readable/` is gitignored). Rebuild it any time from the committed snapshot, with no API calls:

```bash
python scripts/return_map.py            # cache/returns_snapshot.json -> readable/return_map.html
python scripts/return_map.py --sample   # render bundled sample data (smoke test, no snapshot needed)
...
| S&P 500 | `sources/sp500.txt` | S&P 500 constituents — **written by Coverage Manager** weekly (since 2026-09-22); `scripts/refresh_sp500.py` is a manual fallback |
| S&P 500 names | `sources/sp500_names.json` | Wikipedia-sourced ticker → company name map; used as a display fallback for S&P 500 tickers not maintained by Coverage Manager. Written by Coverage Manager with `sp500.txt` |
...
| 30Y mortgage | _none (FRED constant)_ | 30-year fixed mortgage rate — Freddie Mac PMMS via FRED `MORTGAGE30US` (`MORTGAGE_SERIES_ID` in `sigma_screener.py`). Renders as the **last row of `_Macro_`**. **Weekly series** (published Thursday), so the change is labeled `w/w` and the row carries its observation date. No source file and no API key |
| Credit | _none (FRED constants)_ | HY & IG effective yields + OAS spreads from FRED's public no-key CSV (`CREDIT_SERIES` in `sigma_screener.py`). Render under `_Credit_` after `_Macro_`. No source file and no API key |
| Index ETFs | `sources/index_etfs.txt` | Broad-market benchmarks (SPYM, DIA, QQQ, `^W5000` Wilshire 5000, `^RUT` Russell 2000). Render under `_US Indices_`, which **leads** the **Index, Sector & Macro Returns** Slack section |
| Global equity ETFs | `sources/global_equity_etfs.txt` | Global/international equity index ETFs (ACWI, EFA, EEM, VGK, EWJ, EWY, FXI, INDA). Render under `_Global Equity_` below the US `_US Indices_` group. Country ETFs are USD-denominated (bundle local-equity + FX) |
...
| Commodity ETFs | `sources/commodity_etfs.txt` | Commodity ETFs — `GLD` (Gold). Render under a `_Commodities_` sub-header below `_Tech Themes_` via the normal ETF formatter (prior-year/YTD returns) |
| ETF names | `sources/etf_names.json` | `{TICKER: friendly name}` for index + global-equity + sector + healthcare + tech + commodity + macro tickers; pharma rows labeled by composition (large-cap vs diversified). Coverage Manager doesn't maintain this metadata, so display names live here and get merged into the metadata copy at startup |
