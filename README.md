...
Each screener run writes `cache/returns_snapshot.json` (the returns block packed to JSON) and rebuilds `readable/return_map.html`, three ranked columns (Prior Year / YTD / Today) with tiles colored by asset class, hover detail, and a click-to-isolate legend. The HTML is self-contained (no CDN/libraries — opens straight off disk, works offline). Snapshot persistence is warm-fail: if it errors, the alert pipeline is unaffected.
