...
Each alert line includes a direction marker (🟩 up / 🟥 down), ticker, short company name, percent move, z-score, price, 52-week low–high range, and year-to-date return (vs the prior calendar year-end close):

```
🟩  *ISRG* (Intuitive Surgical)  |  +3.25%  |  z = +2.45  |  $485.20  |  52w: $310.40 - $502.10  |  YTD: +18.40%
```

The YTD figure reuses the already-downloaded close history in the full-screen path (no extra download) and the cached prior year-end close in the morning cached-open path; lines for recent IPOs that don't span the prior year render without it.
